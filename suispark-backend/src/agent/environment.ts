...
const twitterUsernameSchema = z
  ...
  .regex(/^[A-Za-z0-9_]*$/, 'An X Username can only contain letters, numbers, and underscores');
...
export async function validateTwitterConfig(runtime: IAgentRuntime): Promise<any> {
  try {
    const twitterConfig = {
      TWITTER_DRY_RUN: parseBooleanFromText(runtime.getSetting('TWITTER_DRY_RUN') || process.env.TWITTER_DRY_RUN) ?? false, // parseBooleanFromText return null if "", map "" to false
      ...
      // number as string?
      MAX_TWEET_LENGTH: safeParseInt(runtime.getSetting('MAX_TWEET_LENGTH') || process.env.MAX_TWEET_LENGTH, DEFAULT_MAX_TWEET_LENGTH),

      TWITTER_SEARCH_ENABLE: parseBooleanFromText(runtime.getSetting('TWITTER_SEARCH_ENABLE') || process.env.TWITTER_SEARCH_ENABLE) ?? false,

      // string passthru
      TWITTER_2FA_SECRET: runtime.getSetting('TWITTER_2FA_SECRET') || process.env.TWITTER_2FA_SECRET || '',

      // int
      TWITTER_RETRY_LIMIT: safeParseInt(runtime.getSetting('TWITTER_RETRY_LIMIT') || process.env.TWITTER_RETRY_LIMIT, 5),
      ...
      TWITTER_TARGET_USERS: parseTargetUsers(runtime.getSetting('TWITTER_TARGET_USERS') || process.env.TWITTER_TARGET_USERS),
      ...
      // bool
      ENABLE_ACTION_PROCESSING: parseBooleanFromText(runtime.getSetting('ENABLE_ACTION_PROCESSING') || process.env.ENABLE_ACTION_PROCESSING) ?? false,
      ...
      // bool
      POST_IMMEDIATELY: parseBooleanFromText(runtime.getSetting('POST_IMMEDIATELY') || process.env.POST_IMMEDIATELY) ?? false,

      TWITTER_SPACES_ENABLE: parseBooleanFromText(runtime.getSetting('TWITTER_SPACES_ENABLE') || process.env.TWITTER_SPACES_ENABLE) ?? false,
    };
